...
void main() {
  test('parses a manifest and keeps only safe passenger fields', () {
    final manifest = DriverManifest.fromJson({
      'trip': {'total_capacity': 4, 'booked_seats': 2, 'available_seats': 2, 'status': 'scheduled'},
      'bookings': [
        {'id': 1, 'booking_reference': 'TE-TEST', 'seats': 2, 'status': 'confirmed', 'passenger': {'name': 'Ahmed'}},
      ],
    });
  ...
  test('malformed optional values use safe defaults', () {
    final manifest = DriverManifest.fromJson({'bookings': [{}]});
